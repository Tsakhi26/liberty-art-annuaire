...
export default function StudentCard({ student, isAdmin = false, onEdit }) {
  ...
  const handleCardClick = () => {
    router.push(`/artist/${student.id}`)
  }

  return (
    <div
      onClick={handleCardClick}
      className="relative bg-white rounded-lg shadow-lg p-6 text-center hover:shadow-xl transition-all hover:-translate-y-1 cursor-pointer h-full flex flex-col"
    >
      {nouveau && (
      ...
      <div className="flex justify-center items-center gap-4 shrink-0 min-h-[2.5rem]">
        {student.insta_url && (
