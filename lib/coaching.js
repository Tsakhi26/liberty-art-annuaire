...
export function getCoachingProgress(student) {
  if (!student.date_debut_coaching) {
    return { percent: 0, currentMonth: 0, totalMonths: 6, daysLeft: 0, daysTotal: 0, daysElapsed: 0, isNearEnd: false, isComplete: false, hasData: false }
  }
  ...
  const currentMonth = Math.min(totalMonths, Math.max(1, Math.ceil((elapsedDays / totalDays) * totalMonths)))
